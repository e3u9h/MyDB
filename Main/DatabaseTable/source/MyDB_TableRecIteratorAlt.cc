...
bool MyDB_TableRecIteratorAlt :: advance () {
	...
	if (myParent[curPage].getType () == MyDB_PageType :: RegularPage && myIter->advance ())
		return true;
	// cout << "myTable-> lastPage " << myTable->lastPage () << endl;
	if (curPage == myTable->lastPage () || curPage == highPage)
		return false;

	curPage++;
	// cout << "DEBUG: Moving to page " << curPage << endl;
	MyDB_PageReaderWriter accessor = myParent[curPage];
	// cout << "DEBUG: Accessed page " << curPage << endl;
	myIter = accessor.getIteratorAlt ();
	// cout << "DEBUG: Got iterator for page " << curPage << endl;
	return advance ();
}
