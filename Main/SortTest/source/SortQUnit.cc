...
int main (int argc, char *argv[]) {
	...
	cout << "Sort a table.."  << endl << flush;
	{
		...
		MyDB_TableReaderWriter outputTable (outTable, myMgr);
		// cout << "Sorted empty table. outputTable-> lastPage " << outputTable.getTable()->lastPage() << endl << flush;
		...
		MyDB_RecordIteratorAltPtr myIter = outputTable.getIteratorAlt ();
		// cout << "Got iterator. outputTable-> lastPage " << outputTable.getTable()->lastPage() << endl << flush;
		// there should be 0 records
		int counter = 0;
		while (myIter->advance ()) {
			// cout << "Advancing iterator." << endl << flush;
			myIter->getCurrent (rec1);
			counter++;
		}
		cout << "Counted records." << endl << flush;
